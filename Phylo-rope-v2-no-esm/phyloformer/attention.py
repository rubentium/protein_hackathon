...
from phyloformer.rotary_embedding import RotaryEmbedding
...
class BaseAttention(nn.Module):
    ...
    def __init__(
        ...
        if embed_dim % nb_heads != 0 or qk_dim % nb_heads != 0:
            raise ValueError(
                "Embed dim and QK dim (if specified) must tbe divisible by the number of heads.\n"
        ...
        # print("PRINTING: A43", f"EMBED: {self.embed_dim}    QK: {self.qk_dim}  #HEADS:   {self.nb_heads}  DHEADS:   {self.head_dim} DHEADS QK: {self.head_qk_dim}")

        self.rot_block = RotaryEmbedding(self.head_qk_dim)
...
class ScaledLinearAttention(BaseAttention):
    ...
    def __init__(
        ...
        super().__init__(nb_heads, embed_dim, nb_heads*2, dropout)
