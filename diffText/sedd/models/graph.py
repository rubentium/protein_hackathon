...
class Graph(abc.ABC):
    ...
    #     # Ensure sigma is properly broadcasted to match i's shape
    #     print("i", i.shape)
    #     print("sigma", sigma.shape)
    #     sigma = unsqueeze_as(sigma, i)
    #     print("sigma after unsqueeze", sigma.shape)
    #     move_chance = 1 - (-sigma).exp()
    #     print("move_chance", move_chance.shape)
        
    #     # print("move_chance", move_chance.shape)
    #     # print("i", i.shape)
        
    #     move_indices = torch.rand(*i.shape, device=i.device) < move_chance
    #     print("move_indices", move_indices.shape)
    #     # print("move_indices", move_indices)
    #     i_pert = torch.where(move_indices, self.dim - 1, i)
    #     print("i_pert", i_pert.shape)
...
class AbsorbingGraph(Graph):
    ...
    #     return i_pert
    def sample_transition(self, i, sigma, mask=None):
        # Ensure sigma is properly shaped to match i
        # i has shape [B, L, D], sigma should be [B, L]
        if sigma.dim() == 1:
            # If sigma is [B], expand to [B, L]
            sigma = sigma.unsqueeze(1).expand(i.shape[0], i.shape[1])
        elif sigma.dim() == 2 and sigma.shape != i.shape[:2]:
            # If sigma is [B, L] but wrong shape, reshape
            sigma = sigma.expand(i.shape[0], i.shape[1])
        
        # Now sigma should have shape [B, L] to match i's first two dimensions
        move_chance = 1 - (-sigma).exp()
        
        # Generate random tensor with same shape as i's first two dimensions
        move_indices = torch.rand(i.shape[0], i.shape[1], device=i.device) < move_chance
        
        # Expand move_indices to match i's full shape [B, L, D]
        move_indices = move_indices.unsqueeze(-1).expand_as(i)
        ...
        if mask is not None:
            ...
            # Expand mask to match i's shape if needed
            if mask.dim() == 2:
                mask = mask.unsqueeze(-1).expand_as(i)
            i_pert = torch.where(mask, i, i_pert)
